...
class UserGenerator(object):
    def __init__(self, fake_string="", age_from=0, age_to=100, second_name=None, used_names=set()):
        ...
        self.male_names = map(lambda s: self.fake_string + s, data['male'])
        self.female_names = map(lambda s: self.fake_string + s, data['female'])
        self.second_names = map(lambda s: self.fake_string + s, data['second_names'])
        self.sentences = map(lambda s: self.fake_string + s, data['sentences'])
        self.streets = map(lambda s: self.fake_string + s, data['streets'])
        self.cities = map(lambda s: self.fake_string + s, data['cities'])
    ...
    def generate_user(self):
        ...
        return {
            ...
            'email': self.fake_string + 'testing+%s_%s@tutorcruncher.com' % (first_name, second_name),
